...
install: ## Install dependencies
	@echo "Installing the 'greynir_client' project..."
	...
	@echo "Installed the 'greynir_client' project. You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:alexandrainst/greynir_client.git; \
	fi

docs:  ## Generate documentation
	@poetry run pdoc --docformat google src/greynir_client -o docs
...
view-docs:  ## View documentation
	...
	@uname=$$(uname); \
		...
		"$${openCmd}" docs/greynir_client.html

test:  ## Run tests
	@poetry run pytest && poetry run readme-cov

docker:  ## Build Docker image and run container
	@docker build -t greynir_client .
	@docker run -it --rm greynir_client
